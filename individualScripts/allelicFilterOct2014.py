...
def remove_multiAlignments(Read, KeepInfo):
    ...
    # that score is decremented by some amount --> can result in negative values
    try:
    ...
    # then I interpret that read as multimapping
    # caution: bowtie2 < version 2.0. had a bug in setting XS tags for paired-end data
    # cuation II: tophat uses the XS flag to indicate which strand the read originated from (+/-)
    try:
